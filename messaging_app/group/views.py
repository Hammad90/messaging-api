...
from .models import Groups, GroupMembers, MemberRole
...
class JoinGroupAPIView(APIView):
    def post(self, request: Request):
        ...
        if GroupMembers.objects.filter(user=user, group=group, role=MemberRole.ADMIN).exists():
            return Response(
        ...
        return Response(
            {'status': 'User has joined the group'},
            status=HTTP_200_OK
        )
...
class LeaveGroupAPIView(APIView):
    def post(self, request: Request):
        ...
        group_member = GroupMembers.objects.filter(
            user=user,
            group=group
        ).first()
        if not group_member:
        ...
        group_member.delete()
        return Response(
            {'status': f'{user.name} has left the group'},
            status=HTTP_200_OK
        )
